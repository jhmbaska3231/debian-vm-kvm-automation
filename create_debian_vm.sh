...
expand_lvm_filesystem() {
    ...
    echo "Step 1: Detecting partition table type and growing partition..."
    PARTITION_TABLE=$(parted /dev/${ROOT_DEV} print 2>/dev/null | grep "Partition Table:" | awk '{print $3}')
    echo "Partition table type: $PARTITION_TABLE"
    
    if [ "$PARTITION_TABLE" = "msdos" ]; then
        echo "MBR partition table detected, checking for extended/logical partition layout..."
        
        # Check if partition is a logical partition (number >= 5 means logical in MBR)
        if [ "$ROOT_PART_NUM" -ge 5 ]; then
            echo "Logical partition detected (vda${ROOT_PART_NUM}), extended partition expansion required..."
            
            # Find the extended partition number (type 'extended' in parted output)
            EXTENDED_PART_NUM=$(parted /dev/${ROOT_DEV} print 2>/dev/null | awk '/extended/ {print $1}')
            
            if [ -n "$EXTENDED_PART_NUM" ]; then
                echo "Found extended partition: /dev/${ROOT_DEV}${EXTENDED_PART_NUM}"
                
                # Expand extended partition first
                echo "Expanding extended partition /dev/${ROOT_DEV}${EXTENDED_PART_NUM} to 100%..."
                if parted /dev/${ROOT_DEV} resizepart ${EXTENDED_PART_NUM} 100% 2>/dev/null; then
                    echo "Extended partition expanded successfully"
                else
                    echo "WARNING: Extended partition expansion failed or already at maximum size"
                fi
                
                # Now expand the logical partition inside it
                echo "Expanding logical partition /dev/${ROOT_DEV}${ROOT_PART_NUM} to 100%..."
                if parted /dev/${ROOT_DEV} resizepart ${ROOT_PART_NUM} 100% 2>/dev/null; then
                    echo "Logical partition expanded successfully"
                else
                    echo "WARNING: Logical partition expansion failed or already at maximum size"
                fi
            else
                echo "WARNING: Could not find extended partition, attempting direct growpart..."
                growpart /dev/$ROOT_DEV $ROOT_PART_NUM 2>/dev/null || echo "growpart failed or already at maximum size"
            fi
        else
            echo "Primary partition detected, using growpart..."
        ...
        echo "GPT partition table detected, using growpart..."
        if growpart /dev/$ROOT_DEV $ROOT_PART_NUM 2>/dev/null; then
            echo "Partition grown successfully"
        else
            echo "Partition growth failed or already at maximum size"
...
echo "  - Set up networking (systemd-networkd/NetworkManager)"
